	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/order_settlement/table','add') || $check_action('/order_settlement/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/order_settlement/table','del') || $check_action('/order_settlement/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="number" @sort-change="$sortChange" label="编号"
				v-if="user_group == '管理员' || $check_field('get','number')" min-width="200">
					</el-table-column>
					<el-table-column prop="name" @sort-change="$sortChange" label="名称"
				...
				v-if="user_group == '管理员' || $check_field('get','purchase_quantity')" min-width="200">
					...
					<el-table-column prop="total_amount" @sort-change="$sortChange" label="合计金额"
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/order_settlement/table','set') || $check_action('/order_settlement/view','set') || $check_action('/order_settlement/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/order_settlement/table','set') || $check_action('/order_settlement/view','set') || $check_action('/order_settlement/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/order_settlement/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/order_settlement/get_list?like=0",
				url_del: "~/api/order_settlement/del?",
		...
		methods: {
			...
			get_user_user_no(id){
				...
				return ret;
															...
															openPayModal(v){
				this.pay_obj.id = v.order_settlement_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/order_settlement/set?order_settlement_id="+this.pay_obj.id);
