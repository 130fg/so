		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/reservation_information/table','add') || $check_action('/reservation_information/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/reservation_information/table','del') || $check_action('/reservation_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','check_in_time')" min-width="200">
		                <template slot-scope="scope">
                	{{ $toTime(scope.row["check_in_time"],"yyyy-MM-dd") }}
                </template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/reservation_information/table','set') || $check_action('/reservation_information/view','set') || $check_action('/reservation_information/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
				...
				<template slot-scope="scope">
					...
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/reservation_information/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
		...
		methods: {
			...
			get_user_user_no(id){
				...
				return ret;
													...
													openPayModal(v){
				this.pay_obj.id = v.reservation_information_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/reservation_information/set?reservation_information_id="+this.pay_obj.id);
