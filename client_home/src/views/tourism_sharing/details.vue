<template>
	<div class="diy_details page_tourism_sharing" id="tourism_sharing_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_tourism_sharing'>
							<div_tourism_sharing :obj="obj"></div_tourism_sharing>
...
<script>
	import div_tourism_sharing from "@/components/diy/div_tourism_sharing.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "tourism_sharing_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/tourism_sharing/set?tourism_sharing_id=' + obj["tourism_sharing_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "tourism_sharing",
							source_field: "tourism_sharing_id",
							source_id: this.obj.tourism_sharing_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.tourism_sharing_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "tourism_sharing",
					source_field: "tourism_sharing_id",
					source_id: obj["tourism_sharing_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/tourism_sharing/set?tourism_sharing_id=' + _this.obj["tourism_sharing_id"], {
								praise_len
							}, (res) => {
								if(res.result){
