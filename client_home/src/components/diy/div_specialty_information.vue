<template>
	<div class="diy_details_box diy_div_specialty_information">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/order_settlement/edit','add')" @click="to_form('/order_settlement/edit')" > 订购 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/specialty_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/specialty_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "specialty_information_id",
					source_id: this.obj["specialty_information_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/specialty_information/set?specialty_information_id=' + _this.obj["specialty_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var specialty_information_id = this.obj.specialty_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "specialty_information",
          source_field: "specialty_information_id",
          source_id: specialty_information_id
        }, (res) => {
